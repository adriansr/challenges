...
class CandyProblem {
        ...
        int maximize() {
            ...
            for (int candy_id = 0; candy_id < ncandy_ ; ++candy_id) {
                ...
                if (candy.count>0 && money >= candy.value) {
                    ...
                    if (cost < INF) {
                    if (purchases.empty() || cost < best_remainder) {
                        purchases.clear();
                        p.coin_combination = choice.first;
                        purchases.push_back( p );
                        best_remainder = cost;
                        if (candy.count>1) {
                            best_count = maximize_purchase(candy.count,choice.first);
                        }
                        else 
                            best_count = 1;
                    }
                    else if (best_remainder == cost) {
                        int count = 1;
                        if (candy.count>count)
                            count = maximize_purchase(candy.count,choice.first);
                        if (count == best_count) {
                            p.coin_combination = choice.first;
                            purchases.push_back( p );
                        }
            ...
            for ( const auto& purchase : purchases) {
                ...
                const auto& candy = vcandy_[purchase.candy_id];
                const auto& coins_used = purchase.coin_combination;
    ...
    private:
        ...
        using ChoiceResult = std::pair<Vi,int>;
        ...
        int choices_for_cost_it(int cost, Vi state, int coin_id, Vi& retval,int best) const {
