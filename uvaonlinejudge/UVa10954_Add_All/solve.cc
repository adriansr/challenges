...
int main() {
    ...
    for (;;) {
        ...
        container.reserve(N);
        std::priority_queue<int,std::vector<int>,std::greater<int>> queue(std::greater<int>(),container);

        reader.get_line(line);
        helper::for_each_number<int>(line,[&queue](int num) {
            queue.push(num);
        });
        ...
        for (int i=1;i<N;++i) {
            ...
            int partial_cost = a + b;
            cost += partial_cost;
            queue.push(partial_cost);
        }
