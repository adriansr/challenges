...
class LineReader {
	...
	private:
		bool fill_buffer() {
			...
			if (remain) {
				if (remain < (len>>1)) {
					std::memcpy(&buffer_[0],&buffer_[used_],remain);
				}
				else {
					std::memmove(&buffer_[0],&buffer_[used_],remain);
				}
			}
			used_ = remain;
			auto ret = read(0,&buffer_[used_],fit);
			if (ret == fit) {
				return true;
			}
			if (ret>0) {
				buffer_.resize(used_ + ret);
				return true;
			}
			return false;
		}
...
#define CCHECK(X...) continue_flag |= X ;
	...
	CCHECK(update_partial<1,0,0,1>(last_col,0));
	CCHECK(update_partial<0,1,1,0>(0,last_row));
	CCHECK(update_partial<1,1,0,0>(last_col,last_row));

	for (int x = 1;x<last_col;++x) {
		CCHECK(update_partial<1,0,1,1>(x,0));
		CCHECK(update_partial<1,1,1,0>(x,last_row));
	}
	for (int y = 1;y<last_row;++y) {
		CCHECK(update_partial<0,1,1,1>(0,y));
		CCHECK(update_partial<1,1,0,1>(last_col,y));
	}

	int pos = num_cols + 1;
	for (int y=1;y<last_row;++y,pos+=2) {
		for (int x=1;x<last_col;++x,++pos) {
