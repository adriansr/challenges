    ...
    for (int test_num = 1; test_num <= num_tests; ++test_num) {
        ...
        for (int idx=0,len=line.size();idx<len;idx+=2) {
            ...
            if (state.num_neighbours[node]>0) {
                if (!state.colored[node]) {
                    ++ntrees;
                    int pending[N],
                        pending_index;

                    for (pending_index=1,pending[0]=node;pending_index>0;) {
                        int sel = pending[--pending_index];
                        state.colored[sel] = true;
                        for (int j=0,remain=state.num_neighbours[sel];remain>0 && j<N;++j) {
                            if (state.g[sel][j]) {
                                -- remain;
                                if( !state.colored[j]) {
                                    state.colored[j] = true;
