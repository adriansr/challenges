...
constexpr int DIFFERENT_MIN = 1234,
              DIFFERENT_MAX = 98765;

bool is_different(const std::string& a) {

    char digit_count[10] = {0};

    for (int i = 0, len = a.size(); i < len; ++i) {
        if ( ++digit_count[ a[i] - '0'] > 1) {
            return false;
        }
    }
    return true;
...
int main() {
    ...
    // lets fill out a map containing all numbers of 'abcde' type,
    // I'll call them "different" numbers
    std::map<int,std::string> different_numbers;

    std::string num("01234");
    for (int i = DIFFERENT_MIN; i <= DIFFERENT_MAX ; ++i) {
                
        if (is_different(num)) {
            different_numbers[i] = num;
        }
    ...
    int N;
    for (int test_count = 0
        ; reader.get_line(line) && (N = helper::string_to_int(line))>0
        ; ++test_count) {
        ...
        terminator.append(Ns).push_back('\n');

        // for every input number, find all different numbers that also
        // multiply to a different number. 
        bool found_any = false;

        for ( const auto it : different_numbers) {
            const int A = it.first,
                      B = A * N;
            
            if ( B <= DIFFERENT_MAX) {
                const auto it2 = different_numbers.find(B);
                if (it2 != different_numbers.end() && is_different(it2->second + it.second)) {
                    output.append(it2->second)
                          .append(" / ",3)
                          .append(it.second)
                          .append(terminator);
                    found_any = true;
        ...
        if (!found_any) {
            output.append("There are no solutions for ")
                  .append(Ns)
                  .append(".\n",2);
        }
    }
